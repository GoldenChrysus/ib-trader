...
def rebalance_holdings():
    ...
    free_cash = data['cash'] * 0.98
    total_value = free_cash + data['total']
    orders = []
    spent = 0.0
    ...
    for symbol in data['allocations'].keys():
        ...
        viable_shares = min(ideal_shares, round_down((free_cash - spent) / data['prices'][symbol], 4))
        limit = data['prices'][symbol] + 0.05
        ...
        spent += (float(viable_shares) * limit)
